...
the alt tag is for providing information about the image for users who cannot view the image.
...
> The color should be without quotes,
background-color = red;
...
```rb
student = student.new(name: "Geraldo", id: 1);
pbj = Sandwich.new
...
@artist.save! gets created in the database or if it already exists it will get updated.
...
```js
var Instructor = function Instructor(info) {
  this.name = info.name,
...
Option C is the most correct because the file names are properly aligned with the folder names. It allows for others to easily locate the files that they need without having to search through many folders.
...
```js
I am kind of unsure of how to convert AR to mongoose. 
```
